    ...
    if (update.message && !update.message.photo) {
      ...
      if (text.startsWith('/check') || text === 'задания') {
        ...
        for (const task of subTasks) {
          ...
          if (!task.channel_username) continue;
        ...
        if (rewardedCount > 0) {
          ...
          await sendTelegramMessage(
            ...
            `Подпишитесь на каналы и попробуйте снова командой /check`
          );
        } else {
          await sendTelegramMessage(chatId, '✅ Все доступные задания уже выполнены.');
        }
